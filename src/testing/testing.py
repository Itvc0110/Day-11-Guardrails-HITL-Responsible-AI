...
async def run_comparison():
    ...
    print("=" * 60)
    _init_judge()  # Initialize the safety judge
    ...
    # Mark as blocked if input/output guardrails blocked it
    for result in protected_results:
        if any(kw in result.get("response", "").lower() for kw in ["cannot", "block", "inappropriate", "off-topic"]):
            result["blocked"] = True
...
class SecurityTestPipeline:
    ...
    async def run_all(self, attacks: list = None) -> list:
        ...
        # TODO 11: Run all attacks in the pipeline
        results = []
