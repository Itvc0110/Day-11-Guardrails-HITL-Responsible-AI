...
# TODO 7: Create safety_judge_agent
...
class OutputGuardrailPlugin(base_plugin.BasePlugin):
    ...
    async def after_model_callback(
        ...
        *,
        ...
        # 2. Use LLM judge to check safety
        if self.use_llm_judge:
            judge_result = await llm_safety_check(response_text)
            if not judge_result["safe"]:
                self.blocked_count += 1
                # Replace with safe message
                safe_msg = "I apologize, but I cannot provide that information as it may contain sensitive data or inaccurate information. Please ask another question."
                llm_response.content = types.Content(
                    role="model",
